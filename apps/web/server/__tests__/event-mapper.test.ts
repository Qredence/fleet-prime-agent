...
describe("event-mapper", () => {
	...
	it("emits turn_start/turn_end as state events", () => {
		const state = createEventMapperState()
		expect(
			mapAgentSessionEvent(state, { type: "turn_start" } as AgentSessionEvent),
		).toEqual([{ type: "state", state: { name: "turn_start" } }])
		expect(
	...
	it("translates text_delta into a delta frame", () => {
		...
		expect(frames).toEqual([
			{ type: "delta", text: "hello", messageId: `${state.runId}-a0` },
		])
	})
	...
	it("translates thinking_delta into a thinking frame", () => {
		...
		expect(frames).toEqual([
			{ type: "thinking", text: "…", messageId: `${state.runId}-a0` },
		])
	})
	...
	it("emits agent_end with a done frame carrying the finished message", () => {
		...
		const done = frames.find((f) => f.type === "done") as Extract<
			ChatStreamEvent,
			{ type: "done" }
		>
