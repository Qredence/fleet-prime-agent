// @generated by web/design/scripts/generate-openui-contract.ts — do not edit
...
export const openUIPromptSpec = {
	components: {
		...
		PanelAction: {
			signature:
				'PanelAction(label: string, panel: "resources" | "workspace" | "artifacts" | "repl" | "subagents" | "session-insights" | "session-tree", relativePath?: any, focus?: boolean)',
			description:
		...
		LineChart: {
			signature:
				"LineChart(title: string, description?: string, xAxisKey: string, series: {dataKey: string, label: string, color?: string}[], data: Record<string, string | number>[])",
			description: "A line chart for trends over time or ordered categories.",
		},
		DonutChart: {
			signature:
				"DonutChart(title: string, description?: string, segments: {label: string, value: number, color?: string}[], centerLabel?: string)",
			description: "A donut/pie chart for part-of-whole composition.",
		},
		DataTable: {
			signature:
				'DataTable(title?: string, columns: {key: string, label: string, type?: "text" | "number" | "currency" | "percent", align?: "left" | "right"}[], rows: Record<string, string | number>[])',
			description:
				"A data table with typed columns (text, number, currency, percent), formatted values, and click-to-sort headers.",
		},
		MetricGroup: {
			signature:
				'MetricGroup(metrics: {label: string, value: string, delta?: string, deltaTone?: "up" | "down" | "neutral", sparkline?: number[]}[])',
		...
		Disclosure: {
			signature: "Disclosure(title: string, content?: any, open?: boolean)",
			description: "A compact progressive-disclosure section for optional details.",
		},
		Todo: {
			signature:
				'Todo(title?: string, items: {label: string, status?: "pending" | "in-progress" | "completed" | "cancelled", detail?: string}[])',
		...
		$defs: {
			...
			Input: {
				...
				properties: {
					...
					type: {
						type: "string",
						enum: ["text", "email", "number", "password", "search"],
					},
					disabled: {
						default: false,
						type: "boolean",
			...
			BarChart: {
				...
				required: ["title", "xAxisKey", "series", "data"],
				additionalProperties: false,
				description: "A bar chart for visualizing comparisons or trends using explicitly provided data points.",
			},
			LineChart: {
			...
			Switch: {
				...
				required: ["name", "checked", "disabled"],
				additionalProperties: false,
				description: "A toggle switch component bound to a boolean reactive state.",
			},
			Modal: {
