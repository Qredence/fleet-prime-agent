...
type Offer = { forValue: string; text: string };
...
export function useComposerInlineCompletion({
	...
	const onDraftChange = useCallback(
		(text: string) => {
			...
			const cached = readCache(cacheKey(sessionId, text));
			if (cached) {
	...
	const ghost = offer ? composerCompletionGhost(offer.forValue, offer.text, "append") : undefined;
