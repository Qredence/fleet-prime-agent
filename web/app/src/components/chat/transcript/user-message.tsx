...
import { isTextPart } from "@/components/chat/utils/chat-message-parts";
import { ImageLightbox } from "@/components/tools/image-lightbox";
		...
		<div className={cn("flex flex-col items-end gap-1", className)}>
			{images.length > 0 &&
				images.map((url, i) => (
					<div
						key={url}
						className={cn(
							"max-w-[200px] rounded-chat-message bg-foreground/4 p-1.5",
					...
					>
						<img
							...
							width={184}
							height={120}
							className="block max-h-[120px] max-w-[184px] rounded-chat-message-inner object-cover"
						/>
					</div>
				))}
			...
			{text && (
				<div className="ms-[70px] max-w-[calc(95%-40px)]">
					<div className="rounded-chat-message bg-muted px-3.5 py-1.5 text-sm text-foreground transition-colors">
						<p className="leading-5 wrap-break-word whitespace-pre-wrap text-pretty">{text}</p>
					</div>
				</div>
			)}
		</div>
