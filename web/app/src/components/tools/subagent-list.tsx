...
import { orderedRlmChildren, rlmStatusIcon } from "@/components/chat/subagents/subagent-utils";
 ...
 */
export function SubagentList({ children, tree, className }: SubagentListProps) {
	const agents = useMemo(() => orderedRlmChildren(children, tree), [children, tree]);
	const active = agents.some(
		(child) => child.status === "queued" || child.status === "running" || child.status === "recovering",
	);
	...
	return (
		<section className={cn("mt-2 rounded-xl border border-border/60 bg-muted/20", className)}>
			...
			>
				...
				<ChevronDown
					className={cn("size-3.5 text-muted-foreground transition-transform", (open || active) && "rotate-180")}
				/>
			</button>
			{open || active ? (
				<div className="space-y-1 border-t border-border/50 px-3 py-2">
					{agents.map((child) => (
						<div
							...
							style={{ paddingLeft: `${Math.max(0, child.depth ?? 0) * 12}px` }}
						>
