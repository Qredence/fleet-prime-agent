...
import { stripSandboxPrefixes } from "@/components/workspace/workspace-path-nav";
...
function getDisplayPath(filePath: string): string {
	if (!filePath) return "";
	const stripped = stripSandboxPrefixes(filePath);
	if (stripped !== filePath) return stripped;
...
export const toolRegistry: Record<string, ToolMeta> = {
	...
	"tool-IPython": {
		...
		title: (part) => {
			const isPending = part.state !== "output-available" && part.state !== "output-error";
			return isPending ? "Running cell" : "Ran cell";
		},
		subtitle: (part) => {
			const code = (part.input?.code || "") as string;
			if (!code) return "";
