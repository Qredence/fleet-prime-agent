...
import { useOAuthLoginFlow } from "@/components/settings/providers/use-oauth-login-flow";
...
export function ProviderOAuthSignIn({
	...
	const { busy, cancel, canStart, login, promptAnswer, setPromptAnswer, start, submitPrompt } = useOAuthLoginFlow({
		onConfigured,
		onOAuthLogin,
		...
		<div className="flex flex-col gap-3">
			...
			{waiting ? (
				<div className="flex flex-col gap-2">
					<p className="text-sm">Sign in with {provider.name} to store OAuth credentials for this account.</p>
					{userCode ? (
						<p className="text-sm">
							Enter this code: <span className="font-mono text-foreground">{userCode}</span>
						</p>
					) : null}
					{instructions && !userCode ? (
						<p className="text-xs text-pretty text-muted-foreground">{instructions}</p>
					) : null}
					{authUrl ? (
						<Button
							type="button"
					...
					{prompt ? (
						<div className="flex flex-col gap-2">
							<p className="text-xs text-pretty text-muted-foreground">{prompt.message}</p>
							<Input
								value={promptAnswer}
				...
				<div className="flex flex-col gap-2">
					{error ? (
						<Alert className="px-3 py-2">
							<AlertDescription className="text-xs text-pretty">{error}</AlertDescription>
						</Alert>
					) : (
