...
import { type ItemRect, useFluidHover } from "@/hooks/use-fluid-hover";
...
function useMenuScope(
	...
	const { activeIndex, setActiveIndex, itemRects, isMeasured, sessionRef, handlers, registerItem } = useFluidHover(
		containerRef,
		{ isItemDisabled: rowSkipped },
	);
	...
	const recomputeActive = useCallback(() => {
		const next = orderedRowsRef.current.filter((el) => activeMapRef.current.get(el) && !rowSkipped(el));
	...
	const onFocus = useCallback(
		(e: React.FocusEvent) => {
			...
			const row = target.closest('[data-sidebar="menu-item"],[data-sidebar="menu-sub-item"]') as HTMLElement | null;
			if (!row) return;
			const idx = orderedRowsRef.current.indexOf(row);
			if (idx === -1) return;
			setActiveIndex(idx);
			...
			<AnimatePresence>
				{activeRects.map(({ key, rect, rowChanged }) => (
					<motion.div
						...
						transition={rowChanged ? { ...spring.moderate, opacity: { duration: 0.08 } } : { duration: 0 }}
					/>
				))}
			</AnimatePresence>
			...
			<AnimatePresence>
				{focusRect && (
					<motion.div
						...
						transition={focusRowChanged ? { ...spring.fast, opacity: { duration: 0.08 } } : { duration: 0 }}
					/>
				)}
			</AnimatePresence>
		</>
...
function useMenuRow(rowRef: RefObject<HTMLLIElement | null>, isSubRow = false) {
	...
	const isActiveRow = rowRef.current !== null && (scope?.activeRows.includes(rowRef.current) ?? false);
	...
	const setHasBadge = useCallback(
		(hasBadge: boolean) => setTrailing((prev) => (prev.hasBadge === hasBadge ? prev : { ...prev, hasBadge })),
		[],
	);
	...
	return useMemo(
		...
		[rowRef, attachRow, isHovered, isActiveRow, isSubRow, setActive, setButtonEl, trailing, setActions, setHasBadge],
	);
}
...
function rowGutter(actionCount: number, hasBadge: boolean) {
	...
	const runWidth = (hasBadge ? ROW_SLOT : 0) + actionsWidth + (hasBadge && actionCount ? ROW_GAP : 0);
	const inset = hasBadge ? ROW_BADGE_INSET : ROW_ACTION_INSET;
	return inset + runWidth + ROW_GAP;
}

const SidebarMenuItem = forwardRef<HTMLLIElement, SidebarMenuItemProps>(({ className, children, ...props }, ref) => {
	...
	return (
		<MenuItemContext.Provider value={item}>
			<li ref={refCb} data-sidebar="menu-item" className={cn("group/menu-item relative", className)} {...props}>
				{children}
			</li>
...
const SidebarMenuButton = forwardRef<HTMLButtonElement, SidebarMenuButtonProps>(
	(
		...
		const heightClass =
			size === "sm" ? "h-7" : size === "lg" ? "h-12" : sizeClasses.variant === "compact" ? "h-7" : "h-8";
		...
		const tabIdx = effectiveActive ? 0 : scope?.hasActive ? -1 : row !== null && row === scope?.firstRowEl ? 0 : -1;
		...
		const gutterRest = item?.actionsShowOnHover ? rowGutter(0, item?.hasBadge ?? false) : gutterHover;
		const gutterVars = {
	...
	({ className, showOnHover = false, children, ...props }, ref) => {
		...
		return (
			<div
				...
				{...props}
			>
				<MenuActionsClusterContext.Provider value={true}>{children}</MenuActionsClusterContext.Provider>
			</div>
		);
	},
);
...
const SidebarMenuBadge = forwardRef<HTMLDivElement, SidebarMenuBadgeProps>(({ className, ...props }, ref) => {
	const item = useContext(MenuItemContext);
	const sizeClasses = useSize();
	...
	({ className, showIcon = false, ...props }, ref) => {
		...
		return (
			<div
				...
				className={cn("flex items-center gap-2 px-2", sizeClasses.variant === "compact" ? "h-7" : "h-8", className)}
				{...props}
			>
				{showIcon && (
	...
	({ className, open = true, children, ...props }, ref) => {
		...
		return (
			<motion.div
				...
				initial={false}
				animate={measured ? { height: open ? contentHeight : 0, opacity: open ? 1 : 0 } : { opacity: open ? 1 : 0 }}
				...
				transition={togglingRef.current ? (open ? spring.moderate : spring.moderate.exit) : { duration: 0 }}
				onAnimationComplete={() => {
