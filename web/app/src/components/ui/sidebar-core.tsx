		...
		useEffect(() => {
			...
			const onKeyDown = (event: KeyboardEvent) => {
				...
				if (root.contains(target)) {
					if (mountedProviders.some((el) => el !== root && root.contains(el) && el.contains(target))) return;
					...
					if (wrapped.length > 0) {
						const outermost = wrapped.find((el) => !wrapped.some((other) => other !== el && other.contains(el)));
...
export function slotElement(
	...
	const templateRef =
		(templateProps as { ref?: Ref<HTMLElement> }).ref ?? (template as unknown as { ref?: Ref<HTMLElement> }).ref;
...
function useShortcutKey(): string {
	...
	return shortcut ?? (side === "right" ? SIDEBAR_KEYBOARD_SHORTCUT_RIGHT : SIDEBAR_KEYBOARD_SHORTCUT);
}
...
const SidebarTrigger = forwardRef<HTMLButtonElement, SidebarTriggerProps>(
	({ onClick, size, children, ...props }, ref) => {
		const { toggleSidebar, open, openMobile, isMobile, side, peek, isPeeking, schedulePeek, cancelPeekTimer } =
			useSidebar();
	...
	return (
		<main
			...
			className={cn(
				...
				shape.bgRadius >= 20 ? "peer-data-[variant=inset]:rounded-3xl" : "peer-data-[variant=inset]:rounded-xl",
...
const SidebarInput = forwardRef<HTMLInputElement, SidebarInputProps>(({ className, ...props }, ref) => {
	const shape = useShape();
	const size = useSize();
	return (
		<input
...
const SidebarHeader = forwardRef<HTMLDivElement, SidebarSectionProps>(({ className, ...props }, ref) => (
	<div ref={ref} data-sidebar="header" className={cn("flex shrink-0 flex-col gap-2 p-2", className)} {...props} />
));
SidebarHeader.displayName = "SidebarHeader";

const SidebarFooter = forwardRef<HTMLDivElement, SidebarSectionProps>(({ className, ...props }, ref) => (
	<div
		ref={ref}
		data-sidebar="footer"
...
const SidebarSeparator = forwardRef<HTMLDivElement, SidebarSectionProps>(({ className, ...props }, ref) => (
	<Separator
		ref={ref}
		data-sidebar="separator"
		orientation="horizontal"
		className={cn("mx-2 shrink-0 bg-border", className)}
		{...props}
	/>
));
...
const SidebarGroup = forwardRef<HTMLDivElement, SidebarGroupProps>(
	({ className, collapsible = false, open: openProp, defaultOpen = true, onOpenChange, children, ...props }, ref) => {
		...
		const ctx = useMemo(() => ({ open, toggle, contentId, actionsCount }), [open, toggle, contentId, actionsCount]);
		...
		return (
			<div
				...
				{...props}
			>
				<SidebarGroupContext.Provider value={collapsible ? ctx : null}>{inner}</SidebarGroupContext.Provider>
			</div>
		);
	},
);
...
const SidebarGroupLabel = forwardRef<HTMLDivElement, SidebarGroupLabelProps>(
	({ className, render, asChild, children, ...props }, ref) => {
		...
		let textEnd = 0;
		while (textEnd < nodes.length && (typeof nodes[textEnd] === "string" || typeof nodes[textEnd] === "number")) {
			textEnd++;
		}
...
const SidebarGroupContent = forwardRef<HTMLDivElement, SidebarSectionProps>(({ className, ...props }, ref) => (
