...
import { type HTMLAttributes, type KeyboardEvent, useId, useMemo, useState } from "react";
...
export function Slider({
	...
	const normalized = safeMax === min ? 0 : (clampedValue - min) / (safeMax - min);
	...
	return (
		<div className={cn("relative w-full touch-none", disabled && "opacity-50", className)} {...props}>
			<div
				className={cn(
			...
			>
				...
				<m.div
					aria-hidden="true"
					className={cn("absolute z-[3] w-0.5 rounded-full pointer-events-none")}
					layout
					style={{
						left: handleLeft,
					...
					transition={SPRING_GLIDE}
				/>
				<div aria-hidden="true" className="pointer-events-none absolute inset-0 z-[2] flex items-center px-2">
					{label ? (
						<span className="px-2 text-body text-transparent select-none" aria-hidden="true">
							{label}
						</span>
					) : null}
					<span
						className="ml-auto px-2 text-body text-transparent tabular-nums select-none"
				...
				<div className="pointer-events-none absolute inset-0 z-[4] flex items-center px-2 text-body">
					{label ? (
						<m.span
							className="px-2"
