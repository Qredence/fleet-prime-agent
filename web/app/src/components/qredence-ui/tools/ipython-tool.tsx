	...
	return (
		<div className="flex h-7 items-center justify-between pr-2 pl-2.5">
			<div className="flex min-w-0 items-center gap-1.5 overflow-hidden">
				<span className="inline-flex items-center gap-1 rounded-sm bg-muted px-1.5 py-0.5 font-mono text-micro leading-none text-muted-foreground uppercase">
...
function IpythonBackgroundOutput({ output }: { output: string }) {
	return (
		<div className="mt-1.5 border-t border-dashed border-border/70 pt-1.5 font-mono text-caption leading-[15px]">
			<span className="text-micro font-semibold uppercase tracking-wider text-amber-600/90 dark:text-amber-400/90">
 ...
 */
function IpythonCellCard({ step, state, onComplete, approval, cellIndex, part }: IpythonCellCardProps) {
	...
	const displayOutput = firstNonEmpty(details?.result, details?.stdout, details?.stderr, details?.error?.evalue);
	const hasError = Boolean(details?.error) || Boolean(details?.errorEname) || details?.status === "error";
		...
		<div className="overflow-hidden rounded-chat-tool border border-border bg-muted">
			...
			<div className="overflow-hidden border-t border-border bg-background px-2.5 py-1.5 font-mono text-label leading-[16px]">
				<div className="break-all whitespace-pre-wrap">
					<span className="mr-2 text-cyan-600 select-none dark:text-cyan-400">{cellLabel}:</span>
