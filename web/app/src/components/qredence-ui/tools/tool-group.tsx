...
import { useElapsedTime } from "@/components/qredence-ui/chat/hooks/use-elapsed-time";
import { GenericTool } from "@/components/qredence-ui/tools/generic-tool";
import { toolRegistry } from "@/components/qredence-ui/tools/tool-registry";
import { ToolRowBase } from "@/components/qredence-ui/tools/tool-row-base";
import { formatElapsedTime, getToolStatus } from "@/components/qredence-ui/tools/utils/format-tool";
		...
		<ToolRowBase
			...
			trailingContent={
				showElapsed && elapsedTimeDisplay ? (
					<span className="shrink-0 font-normal text-muted-foreground/60 tabular-nums">{elapsedTimeDisplay}</span>
				) : undefined
			}
		>
			<div className="relative">
				...
				>
					{(isPending ? nestedTools.slice(0, Math.max(visibleCount, 0)) : nestedTools).map((nestedPart, idx) => {
						...
						const { isPending: nestedIsPending, isError: nestedIsError } = getToolStatus(derivedPart, chatStatus);
						return (
							<GenericTool
								key={
									derivedPart.toolCallId ??
									derivedPart.id ??
									`${derivedPart.type}-${nestedMeta.title(derivedPart)}`
								}
