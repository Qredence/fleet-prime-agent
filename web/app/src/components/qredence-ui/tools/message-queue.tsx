...
export type MessageQueueProps = {
	...
	onDelete?: (lane: QueueLane, index: number, text: string) => void | Promise<unknown>;
	onEdit?: (lane: QueueLane, index: number, expectedText: string, nextText: string) => void | Promise<unknown>;
	className?: string;
};

const LANES: ReadonlyArray<{ key: QueueLane; label: string }> = [
	{ key: "steering", label: "Next" },
	{ key: "followUp", label: "After run" },
];

function queueItemKey(lane: QueueLane, index: number): string {
	return `${lane}:${index}`;
}
 ...
 */
export function MessageQueue({ queue, onDelete, onEdit, className }: MessageQueueProps) {
	...
	const startEdit = (lane: QueueLane, index: number, text: string) => {
		setEditingKey(queueItemKey(lane, index));
		setDraftText(text);
	};

	const handleDelete = async (lane: QueueLane, index: number, text: string) => {
	...
	const handleSaveEdit = async (lane: QueueLane, index: number, expectedText: string) => {
	...
	return (
		<section aria-label="Queued messages" className={cn(CHAT_COLUMN_CLASS, "mb-2", className)}>
