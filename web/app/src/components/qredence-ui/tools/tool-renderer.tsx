...
import { QuestionTool } from "@/components/qredence-ui/chat/question/question-tool";
import type { CustomToolRendererProps } from "@/components/qredence-ui/chat/types";
import { BashTool } from "@/components/qredence-ui/tools/bash-tool";
import { EditTool } from "@/components/qredence-ui/tools/edit-tool";
import { GenericTool } from "@/components/qredence-ui/tools/generic-tool";
import { IpythonTool } from "@/components/qredence-ui/tools/ipython-tool";
import { McpTool, unwrapMcpOutput } from "@/components/qredence-ui/tools/mcp-tool";
import { PlanTool } from "@/components/qredence-ui/tools/plan-tool";
import { SearchTool } from "@/components/qredence-ui/tools/search-tool";
import { TodoTool } from "@/components/qredence-ui/tools/todo-tool";
import { ToolGroup } from "@/components/qredence-ui/tools/tool-group";
import { parseMcpToolType, toolRegistry } from "@/components/qredence-ui/tools/tool-registry";
import { getToolStatus } from "@/components/qredence-ui/tools/utils/format-tool";
...
export const ToolRenderer = memo(function ToolRenderer({
	...
	const { isPending, isError } = getToolStatus(part, chatStatus);
	return <GenericTool title={isPending ? `Running ${toolName}` : toolName} isPending={isPending} isError={isError} />;
});
