...
function calculateDiffStatsFromPatch(patches: Array<{ lines?: Array<string> }>): string | undefined {
	let addedLines = 0;
	let removedLines = 0;
...
export function mapToolNameToVariant(toolName: string): "thinking" | "action" | "search" | undefined {
...
export function mapToolInvocationToStep(
	...
	const displayToolName = toolName === "PlanWrite" ? "Plan" : toolName === "TodoWrite" ? "Todo" : toolName;
	...
	if (toolName === "Write") {
		const content =
			typeof result?.content === "string" ? result.content : typeof args?.content === "string" ? args.content : "";
	...
	if (toolName === "WebSearch" || toolName === "web_search" || toolName === "Grep" || toolName === "Glob") {
		step.searchQuery = (args?.query ?? args?.pattern) ? String(args?.query ?? args?.pattern) : undefined;
...
export function resolveToolPartState(partState?: string): "partial-call" | "call" | "result" {
...
export function adaptToolPart(
	...
	const step = mapToolInvocationToStep(part.toolCallId ?? part.id ?? toolName.toLowerCase(), {
		toolName,
