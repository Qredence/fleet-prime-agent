...
import { Ban, Check, ChevronDown, CircleX, Copy, FileCode2, LoaderCircle } from "lucide-react";
import { m, useReducedMotion } from "motion/react";
import { type ReactNode, useCallback, useEffect, useId, useLayoutEffect, useRef, useState } from "react";
import { type AgentCodeLanguage, AgentCodeLine, useAgentCodeTokens } from "@/components/qredence-ui/tools/agent-code";
	...
	return (
		<span
			className={cn(
				...
				type === "added" ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400",
			)}
		>
			{type === "added" ? "+" : "−"}
			{value}
...
export function FileDiff({
	...
	useEffect(() => {
		...
		if (previousStatus.current === "streaming" && status !== "streaming" && collapseOnComplete) {
			setOpen(false);
		}
		...
		<div data-state={status} aria-busy={streaming} className={cn("w-full text-sm", className)}>
			...
			>
				...
				<span className="grid size-4 shrink-0 place-items-center text-muted-foreground/60">
					{streaming ? (
						<LoaderCircle aria-label="Applying changes" className={cn("size-3.5", !reduce && "animate-spin")} />
					) : status === "error" ? (
