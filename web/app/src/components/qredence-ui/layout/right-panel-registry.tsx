...
import { useChatPanelDataContext, useWorkspaceTreeContext } from "@/components/qredence-ui/layout/right-panel-context";
...
const LazyArtifactsPanel = lazy(() =>
	import("@/components/qredence-ui/panels/workspace/artifacts-panel").then(({ ArtifactsPanelContent }) => ({
		default: ArtifactsPanelContent,
	})),
);
const LazyResourcesPanel = lazy(() =>
	import("@/components/qredence-ui/panels/workspace/resources-panel").then(({ ResourcesPanelContent }) => ({
		default: ResourcesPanelContent,
	})),
);
const LazySessionInsightsPanel = lazy(() =>
	import("@/components/qredence-ui/panels/session/session-insights-panel").then(({ SessionInsightsPanel }) => ({
		default: SessionInsightsPanel,
	})),
);
const LazyReplPanel = lazy(() =>
	import("@/components/qredence-ui/panels/workspace/repl-panel").then(({ ReplPanelContent }) => ({
		default: ReplPanelContent,
	})),
);
const LazySubagentsPanel = lazy(() =>
	import("@/components/qredence-ui/panels/subagents/subagents-panel").then(({ SubagentsPanelContent }) => ({
		default: SubagentsPanelContent,
	})),
);
const LazyWorkspacePanel = lazy(() =>
	import("@/components/qredence-ui/panels/workspace/workspace-panel").then(({ WorkspacePanelContent }) => ({
		default: WorkspacePanelContent,
	})),
);
const LazySessionTreePanel = lazy(() =>
	import("@/components/qredence-ui/panels/session/session-tree-panel").then(({ SessionTreePanel }) => ({
	...
	return (
		<Suspense fallback={<PanelFallback />}>
			<LazyReplPanel
				artifactRuns={data.artifactRuns}
				selectedArtifactId={data.selectedArtifactId}
				kernelDiagnostics={data.presentation.kernelDiagnostics}
			/>
		</Suspense>
	);
}
