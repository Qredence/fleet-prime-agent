...
export function ThreadSearch({
	...
	className,
	...props
}: Omit<ComponentProps<"div">, "children" | "threads" | "query" | "activeId" | "onQueryChange" | "onSelect"> & {
	...
	const groups = [...new Set(matches.flatMap((t) => (t.pinned ? [] : [t.group])))];

	const ordered = [
		...pinned,
		...groups.flatMap((group) => matches.filter((thread) => !thread.pinned && thread.group === group)),
	];
	...
	const row = (thread: SearchableThread) => (
		...
		>
			<span className="flex items-center gap-1.5">
				...
				<span className="min-w-0 flex-1 truncate text-body">{thread.title}</span>
			</span>
		...
		>
			<div className={cn(field, "flex items-center gap-2 rounded-xl px-2.5 py-1.5")}>
				...
				<input
					...
					className="text-foreground/85 placeholder:text-foreground/30 min-w-0 flex-1 bg-transparent text-base outline-none md:text-body"
				/>
			</div>
			...
			{groups.map((group) => (
				<div key={group} className="flex flex-col">
					...
					{matches.flatMap((thread) => (!thread.pinned && thread.group === group ? [row(thread)] : []))}
				</div>
			))}

			{matches.length === 0 && (
				<span className="text-foreground/30 px-2 py-4 text-center text-xs">No thread matches “{query}”</span>
			)}
		</div>
	);
}
