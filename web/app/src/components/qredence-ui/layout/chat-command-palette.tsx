...
import { RIGHT_PANEL_LAUNCHER_DEFINITIONS } from "@/components/qredence-ui/layout/right-panel-registry";
import {
	CommandDialog,
	CommandEmpty,
	CommandGroup,
		...
		<CommandDialog open={open} onOpenChange={onOpenChange}>
			...
			<CommandList>
				...
				<CommandGroup heading="Actions">
					<CommandItem onSelect={() => handleSelect(onNewSession)} keywords={["new", "session", "chat"]}>
						<Plus className="mr-2 size-4" />
						<span>New session</span>
					</CommandItem>
					{isStreaming && (
						<CommandItem onSelect={() => handleSelect(onStop)} keywords={["stop", "abort", "cancel"]}>
				...
				<CommandGroup heading="Panels">
					{RIGHT_PANEL_LAUNCHER_DEFINITIONS.map((definition) => {
						...
						return (
							<CommandItem
								key={definition.id}
								onSelect={() => handleSelect(() => onSetRightPanel(definition.id))}
							...
							{sessions.map((session) => {
								...
								return (
									<CommandItem
										...
										onSelect={() => handleSelect(() => onResumeSession(session))}
				...
				<CommandGroup heading="Theme">
					<CommandItem onSelect={() => handleSelect(() => onThemeChange("light"))} keywords={["theme", "light"]}>
						<Sun className="mr-2 size-4" />
						<span>Light</span>
						{themePreference === "light" && <Keyboard className="ml-auto size-3 opacity-50" />}
					</CommandItem>
					<CommandItem onSelect={() => handleSelect(() => onThemeChange("dark"))} keywords={["theme", "dark"]}>
						<Moon className="mr-2 size-4" />
						<span>Dark</span>
						{themePreference === "dark" && <Keyboard className="ml-auto size-3 opacity-50" />}
					</CommandItem>
					<CommandItem onSelect={() => handleSelect(() => onThemeChange("system"))} keywords={["theme", "system"]}>
						<Monitor className="mr-2 size-4" />
						<span>System</span>
						{themePreference === "system" && <Keyboard className="ml-auto size-3 opacity-50" />}
					</CommandItem>
				</CommandGroup>
			</CommandList>
		</CommandDialog>
