...
import { SettingsCommitActions, SettingsPane } from "@/components/qredence-ui/chrome/settings-pane";
import { HIT_AREA_EXPAND_DENSE_CLASS } from "@/components/qredence-ui/chrome/tokens";
import { AddModelsDialog } from "@/components/qredence-ui/panels/config-panel/sections/add-models-dialog";
import { isModelEnabled } from "@/components/qredence-ui/panels/config-panel/shared/model-patterns";
import { ProviderBrandIcon } from "@/components/qredence-ui/panels/config-panel/shared/provider-brand-icon";
import { formatProviderLabel } from "@/components/qredence-ui/panels/config-panel/shared/provider-label";
import type { ConfigModelInfo } from "@/components/qredence-ui/panels/config-panel/shared/types";
...
export function ModelDefaultsSection({
	...
	return (
		<SettingsPane
			...
			description={
				<>
					Models available in chat. Discover from configured providers, add the ones you want, and remove any you
					do not need — changes save automatically. <span className="tabular-nums">{listedModels.length}</span> in
					list
					{modelOptions.length > 0 ? (
						<>
							{" "}
							· <span className="tabular-nums">{modelOptions.length}</span> discovered
						</>
					) : null}
					.
				</>
				...
				{emptyMessage ? (
					...
					groupedListed.map(([provider, models]) => (
						<div key={provider} className="flex flex-col gap-1.5">
							<div className="px-1 text-xs font-medium text-muted-foreground">{providerLabel(provider)}</div>
							{models.map((model) => (
								<ItemRow
									key={model.id}
									...
									trailing={
										<div className="flex items-center">
											{isDefaultModel(model) ? (
												<span className="px-2 text-xs font-medium text-muted-foreground">Default</span>
											) : onSetDefaultModel ? (
												<Button
													type="button"
													variant="ghost"
