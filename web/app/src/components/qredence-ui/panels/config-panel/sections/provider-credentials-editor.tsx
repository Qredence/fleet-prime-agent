...
import {
	isOAuthProvider,
	supportsOAuth,
} from "@/components/qredence-ui/panels/config-panel/sections/provider-credentials-types";
import { ProviderOAuthSignIn } from "@/components/qredence-ui/panels/config-panel/sections/provider-oauth-sign-in";
import { CUSTOM_PROVIDER_PICKER_ID } from "@/components/qredence-ui/panels/config-panel/sections/use-provider-credentials-controller";
import { ProviderBrandIcon } from "@/components/qredence-ui/panels/config-panel/shared/provider-brand-icon";
import { ProviderCredentialFields } from "@/components/qredence-ui/panels/config-panel/shared/provider-credential-fields";
import { PROVIDER_METADATA } from "@/components/qredence-ui/panels/config-panel/shared/provider-metadata";
			...
			<div className="max-h-[min(24rem,50vh)] overflow-y-auto">
				{!pickerHasResults ? (
					<p className="py-6 text-center text-xs text-pretty text-muted-foreground">
						{credentialProvidersLength === 0 ? "No providers available." : "No matching providers found."}
					</p>
				) : (
					<div className="flex flex-col gap-3">
						...
						{filteredPickerConfigured.length > 0 ? (
							<div className="flex flex-col gap-1">
								{filteredPickerAvailable.length > 0 ? (
									<p className="px-2 pt-1 text-caption font-medium tracking-wide text-muted-foreground uppercase">
										Already configured
									</p>
								) : null}
...
export function AddProviderEditorPanel({
	...
	const isCustomTemplate = provider.id === CUSTOM_PROVIDER_PICKER_ID || isCustomProviderId(provider.id);
	const meta =
	...
	return (
		<div className="flex flex-col gap-3">
			...
			<Alert className="px-3 py-2">
				<Info />
				<AlertDescription className="text-xs text-pretty">{meta.help}</AlertDescription>
			</Alert>
		...
		>
			<AlertDialogContent>
				<div className="flex flex-col gap-2">
					<AlertDialogTitle>Remove {confirmRemoveProvider?.name ?? "provider"}?</AlertDialogTitle>
					<AlertDialogDescription>
						This clears stored credentials for this provider from your workspace settings. Active sessions will
