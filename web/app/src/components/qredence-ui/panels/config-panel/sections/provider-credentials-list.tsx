...
import { ItemRow } from "@/components/qredence-ui/chrome/item-row";
import { RowSurface } from "@/components/qredence-ui/chrome/surface";
...
export function ActiveProviderList({
	...
	const { api, apiKey, baseUrl, modelId, models, displayName, showPassword, attemptedSave, canSave } = form;
	...
	return (
		<div className="flex flex-col gap-1.5">
			{providers.map((provider) => {
				...
				const meta =
					PROVIDER_METADATA[provider.id] ?? PROVIDER_METADATA[isCustom ? "custom" : "openai-chat-completions"];
				...
				return (
					<div key={provider.id} className="flex flex-col">
						<ItemRow
							interactive={false}
							icon={
								<ProviderBrandIcon provider={provider.id} api={provider.api} className="text-foreground/80" />
							}
							title={provider.name}
							subtitle={
								oauthOnly
						...
						{isEditing ? (
							<RowSurface tone="inset" padding="md" className="flex flex-col gap-2 border-t border-border/30">
								{oauthOnly ? (
									<ProviderOAuthSignIn
										key={provider.id}
										provider={provider}
									...
									<>
										<ProviderCredentialFields
											...
											onDisplayNameChange={
												isCustom || provider.providerFamily !== undefined ? onDisplayNameChange : undefined
											}
