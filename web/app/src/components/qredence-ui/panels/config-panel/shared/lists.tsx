...
} from "@/components/qredence-ui/panels/config-panel/shared/settings-resource-path";
...
export function CatalogValueList({
	...
	const available = options.filter((option) => !settingsResourceListIncludes(values, option.value));
	...
	return (
		<div className="flex flex-col gap-1.5">
			{values.length === 0 ? (
				<p className="px-1 py-2 text-xs text-pretty text-muted-foreground">Nothing selected yet.</p>
			) : (
				values.map((value) => {
					...
					return (
						<ItemRow
							...
							subtitle={catalogLabel && catalogLabel !== value ? value : undefined}
							trailing={
								<Button
									type="button"
									variant="ghost"
