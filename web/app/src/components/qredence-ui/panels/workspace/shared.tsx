...
import { getResourceChipTitle, resourceKey } from "@/components/qredence-ui/panels/workspace/resource-helpers";
...
export function ResourceNotice({
	...
	return (
		<div className="my-1.5 rounded-sm bg-foreground/5 px-2.5 py-2">
			...
			<p className="mt-1 text-caption leading-4 text-foreground/40">{description}</p>
		</div>
	);
}
	...
	return (
		...
		>
			<div className="flex min-w-0 items-center text-body leading-5 text-foreground/45">
				<span className="truncate underline decoration-foreground/25 underline-offset-2">{label}</span>
				<span className="ml-1 shrink-0 text-caption leading-4 text-foreground/30 tabular-nums no-underline">
					{items.length}
				</span>
			</div>
			<ul
			...
			>
				{items.length === 0 ? (
					...
					items.map((item) => <ResourceChip key={resourceKey(item)} icon={Icon} item={item} stacked />)
				)}
			</ul>
		</section>
	);
	...
	return (
		<li
			className={`max-w-full rounded-lg border border-border/70 bg-background px-2.5 text-body leading-5 text-foreground/80 shadow-[0_1px_4px_-1px_rgba(0,0,0,0.06)] ${
				stacked ? "flex min-h-9 w-full min-w-0 items-center gap-2 py-1.5" : "inline-flex h-7.5 items-center gap-1"
			}`}
			aria-label={title}
		...
		>
			...
			{item.source && (
				<span className="max-w-20 shrink-0 truncate rounded-[5px] bg-foreground/5 px-1.5 py-0.5 text-micro leading-3 text-foreground/35">
					{item.source}
				</span>
			)}
			{item.activationStatus && (
				<span className="max-w-24 shrink-0 truncate rounded-[5px] bg-foreground/5 px-1.5 py-0.5 text-micro leading-3 text-foreground/35">
					{item.activationStatus}
				</span>
			)}
		</li>
