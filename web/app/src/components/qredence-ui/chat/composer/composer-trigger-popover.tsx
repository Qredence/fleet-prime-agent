...
import { ComposerMenu, ComposerMenuItem } from "@/components/qredence-ui/chat/composer/composer-menu";
	...
	const renderItem = (item: ComposerTriggerItem, index: number) => {
		...
		return (
			<ComposerMenuItem
				...
				className={cn(
					"flex min-h-10 cursor-pointer items-start gap-2 rounded-lg px-3 py-2 text-start",
					selected && "bg-accent text-accent-foreground",
				)}
			>
				<span className="mt-0.5 shrink-0 text-muted-foreground">{defaultIcon(kind, item)}</span>
