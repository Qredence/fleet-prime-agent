...
import type { QuestionAnswer, QuestionConfig } from "@/components/qredence-ui/chat/question/question-prompt";
import { QuestionPrompt } from "@/components/qredence-ui/chat/question/question-prompt";
...
export function QuestionTool({ part }: QuestionToolProps) {
	...
	const [prevQuestionIndex, setPrevQuestionIndex] = useState(part.input?.questionIndex);
	...
	return (
		<div className="overflow-hidden rounded-chat-tool border border-border bg-muted">
				...
				{showNavigation && (
					<div className="inline-flex items-center gap-1">
						<button
							...
							onClick={goPrev}
							disabled={!canGoPrev}
							className="inline-flex size-5 items-center justify-center rounded-[4px] hover:bg-secondary disabled:opacity-40"
			...
			{isComplete ? (
				<div className="bg-background px-3 py-2 text-xs text-muted-foreground">{summaryText}</div>
			) : (
				<QuestionPrompt
