...
import type { InputBarProps } from "@/components/qredence-ui/chat/composer/input-bar";
import { InputBar } from "@/components/qredence-ui/chat/composer/input-bar";
import { SessionGenerativeTextRenderer } from "@/components/qredence-ui/chat/generative-text-renderer";
import {
	Message,
	MessageBubble,
	MessageBubbleContent,
	MessageContent,
} from "@/components/qredence-ui/chat/message/message";
import { MessageScroller } from "@/components/qredence-ui/chat/message/message-scroller";
import { buildAssistantElements } from "@/components/qredence-ui/chat/message/message-turns";
import { StreamingResponse } from "@/components/qredence-ui/chat/message/streaming-response";
import { TurnStatus } from "@/components/qredence-ui/chat/message/turn-status";
import { UserMessage } from "@/components/qredence-ui/chat/message/user-message";
import {
	TRANSCRIPT_VIRTUALIZATION_THRESHOLD,
	VirtualizedTurnList,
} from "@/components/qredence-ui/chat/message/virtualized-turn-list";
import type { AgentChatProps } from "@/components/qredence-ui/chat/types";
import { CHAT_COLUMN_CLASS } from "@/components/qredence-ui/chrome/tokens";
import type { QueueLane } from "@/components/qredence-ui/tools/message-queue";
import { normalizeAssistantToolParts } from "@/components/qredence-ui/tools/utils/tool-part-normalizer";
import { type ConversationTurn, groupMessages } from "@/lib/pi/conversation-turns";
import { useUiPreferences } from "@/lib/ui-preferences";
import { cn } from "@/lib/utils";

const LazySessionToolRenderer = lazy(() =>
	import("@/components/qredence-ui/tools/session-tool-renderer").then(({ SessionToolRenderer }) => ({
...
const LazySessionReasoningPanel = lazy(() =>
	import("@/components/qredence-ui/chat/thinking/session-reasoning-panel").then(({ SessionReasoningPanel }) => ({
...
const LazyToolTimeline = lazy(() =>
	import("@/components/qredence-ui/tools/tool-timeline").then(({ ToolTimeline }) => ({
...
const LazyPromptSuggestions = lazy(() =>
	import("@/components/qredence-ui/chat/prompt-suggestions").then(({ PromptSuggestions }) => ({
...
const LazyMessageQueue = lazy(() =>
	import("@/components/qredence-ui/tools/message-queue").then(({ MessageQueue }) => ({
