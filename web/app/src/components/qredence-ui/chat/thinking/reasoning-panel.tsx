...
export interface ReasoningPanelProps {
	...
	visibleSteps: number;
	streaming: boolean;
	/** Optional product-safe status copy while work is active. */
...
export function ReasoningPanel({
	...
	return (
		...
		>
			<CollapsibleTrigger className="group/trigger flex items-center gap-1.5 py-1 text-sm text-muted-foreground outline-none transition-[color,scale] hover:text-foreground/90 focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]">
				<SwapLabel active={streaming ? 0 : 1} className="text-start">
					<>
						...
						{elapsed !== undefined && (
							<span className={cn(mono, "tabular-nums text-muted-foreground/70")}>{elapsed}</span>
						)}
					</>
					<>{restingLabel}</>
				</SwapLabel>
