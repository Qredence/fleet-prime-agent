...
import type { ModelSelectorEffort, ModelSelectorModel } from "@/components/qredence-ui/chat/model-selector";
		...
		<Command className="min-h-0 max-h-[min(55vh,28rem)] bg-transparent" shouldFilter>
			...
			<CommandList className="min-h-0 flex-1 p-1.5">
				...
				{groups.map(([provider, providerModels]) => (
					<CommandGroup key={provider} heading={providerModels[0]?.providerLabel ?? provider}>
			...
			{selectedModel?.reasoning && selectedModel.efforts?.length ? (
				<div className="shrink-0 border-t border-border/70 p-2">
					<Slider
						...
						value={Math.max(
							0,
							selectedModel.efforts.findIndex((option) => option.id === selectedEffort?.id),
						)}
						onChange={(nextValue) => {
