...
export function OpenProjectFolderDialog({
	...
	return (
		<Dialog onOpenChange={onOpenChange} open={open}>
			<DialogContent className="sm:max-w-lg" data-testid="open-project-folder-dialog">
				<DialogHeader>
					<DialogTitle>Open project folder</DialogTitle>
					<DialogDescription>
						Choose the local directory that becomes the prime-agent workspace root.
					</DialogDescription>
				</DialogHeader>
				...
				<div className="flex flex-col gap-3">
					<div className="flex gap-2">
						...
						<Input
							...
							value={pathInput}
						/>
						<Button disabled={loading || submitting} onClick={handleGo} type="button" variant="outline">
							Go
						</Button>
					</div>
					...
					>
						<div className="flex shrink-0 items-center gap-1 border-b border-border/60 px-2 py-1.5">
							...
							<span className="min-w-0 truncate text-xs text-muted-foreground">{browse?.path ?? pathInput}</span>
						</div>
						...
						<div className="min-h-0 flex-1 overflow-y-auto p-1">
							{loading ? (
								...
								<p className="px-2 py-6 text-center text-sm text-muted-foreground">No subfolders</p>
							) : (
