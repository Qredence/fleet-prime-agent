...
export function VirtualizedTurnList<T>({
  ...
  const updateMeasuredHeights = useCallback((entries: readonly ResizeObserverEntry[]) => {
    ...
    if (element && itemKeys.length > 0) {
      ...
      if (anchorKey && anchorIndex > 0) {
        ...
        const offsetDelta =
          (nextOffsets[nextAnchorIndex] ?? 0) - (beforeOffsets[anchorIndex] ?? 0)
  ...
  if (!shouldVirtualize) {
    return <>{items.map((item, index) => <Fragment key={itemKeys[index]}>{renderItem(item, index)}</Fragment>)}</>
  }

  const totalHeight = offsets[offsets.length - 1] ?? 0
  const overscanDistance = Math.max(estimateSize, viewport.height * overscan)
  const firstVisible = findItemAtOffset(
    offsets,
    Math.max(0, viewport.scrollTop - overscanDistance),
    items.length,
  )
