	...
	return (
		<LazyMotion features={domAnimation}>
			...
			<AnimatePresence>
				{focusRect ? (
					<m.div
						className={cn("pointer-events-none absolute z-20 border border-ring", PILL_RADIUS)}
						layout
						initial={false}
						style={{
...
const TabsSubtle = forwardRef<HTMLDivElement, TabsSubtleProps>(
	(
		{ children, selectedIndex, onSelect, idPrefix, activeLabel = false, variant = "default", className, ...props },
		ref,
	) => {
		...
		const selectedRect = selectedIndex >= 0 ? tabRects[selectedIndex] : undefined;
		...
		const tabsSubtleContextValue = useMemo(
			...
			[registerTab, hoveredIndex, selectedIndex, idPrefix, activeLabel, variant, onSelect],
		);

		return (
			<TabsSubtleContext.Provider value={tabsSubtleContextValue}>
				<Tabs
					className="data-horizontal:!flex-row !gap-0.5"
...
const TabsSubtleItem = forwardRef<HTMLButtonElement, TabsSubtleItemProps>(
	({ icon: Icon, label, index, badge, className, "aria-label": ariaLabelProp, onClick, ...props }, ref) => {
		const internalRef = useRef<HTMLButtonElement | null>(null);
		const { registerTab, hoveredIndex, selectedIndex, idPrefix, activeLabel, variant, onSelect } = useTabsSubtle();
		...
		return (
			<TabsTrigger
				...
				className={cn(
					"relative z-10 flex flex-none cursor-pointer items-center border-none !bg-transparent !shadow-none outline-none after:hidden",
