...
import { CartesianGrid, Cell, Line, Pie, PieChart, LineChart as RechartsLineChart, XAxis } from "recharts";
import { z } from "zod";
...
function buildChartConfig(series: Array<{ dataKey: string; label: string; color?: string }>) {
	const config: Record<string, { label: string; color: string }> = {};
	series.forEach((entry, index) => {
		config[entry.dataKey] = {
			label: entry.label,
			color: entry.color ?? CHART_FALLBACK_COLORS[index % CHART_FALLBACK_COLORS.length]!,
		};
	});
	return config;
}
...
export const LineChartDef = defineComponent({
	...
	props: z.object({
		...
		xAxisKey: z.string().describe("The key in the data objects used for the X-axis (e.g. 'month', 'day')"),
		series: z
			.array(
				z.object({
					...
					color: z.string().optional().describe("Optional CSS color override for this series"),
				}),
			)
			.describe("The line series to plot"),
		data: z
			.array(z.record(z.string(), z.union([z.string(), z.number()])))
			.describe("Data rows; ALWAYS generate realistic data when inventing datasets"),
	}),
							...
							<PieChart>
								...
								>
									{valid.map((segment, index) => (
										<Cell key={segment.label} fill={segment.color ?? CHART_FALLBACK_COLORS[index % 5]} />
									))}
								</Pie>
							</PieChart>
						</ChartContainer>
...
export const DonutChartDef = defineComponent({
	...
	props: z.object({
		...
		centerLabel: z.string().optional().describe("Optional label rendered at the donut center"),
	}),
