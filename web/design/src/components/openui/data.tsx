...
const NUMBER_FAMILY_TYPES: ReadonlyArray<DataColumnType> = ["number", "currency", "percent"];
...
export function cycleSortState(key: string, current: SortState | null): SortState | null {
...
export function buildSortComparator(
	...
	return (first: Record<string, string | number>, second: Record<string, string | number>) => {
		const firstValue = first[column.key];
		const secondValue = second[column.key];
		if (type === "text") {
			return direction * textCollator.compare(String(firstValue ?? ""), String(secondValue ?? ""));
		}
...
function resolveColumnAlign(column: DataColumn) {
	...
	return column.type && NUMBER_FAMILY_TYPES.includes(column.type) ? "right" : "left";
}
...
function DataTableComponent({
	...
	let sortedRows = rows;
	if (sort) {
		const activeColumn = columns.find((column) => column.key === sort.key) ?? { key: sort.key };
				...
				<div className="w-full overflow-x-auto rounded-lg border">
					<Table className="min-w-max">
								...
								{columns.map((column) => {
									...
									return (
										<TableHead key={column.key} className={alignClasses[align]}>
											<button
												type="button"
												onClick={() => setSort((current) => cycleSortState(column.key, current))}
												className={cn(
						...
						<TableBody>
							{rows.length === 0 ? (
								<TableRow>
									<TableCell colSpan={Math.max(columns.length, 1)} className="text-center text-foreground/40">
										No data
									</TableCell>
								</TableRow>
							) : (
										...
										{columns.map((column) => {
											...
											return (
												<TableCell
													...
													className={cn(
														alignClasses[align],
														column.type && NUMBER_FAMILY_TYPES.includes(column.type) && "tabular-nums",
													)}
												>
...
export const DataTableDef = defineComponent({
	...
	props: z.object({
		...
		columns: z
			.array(
				z.object({
					key: z.string().describe("The key in each row object for this column"),
					...
					align: z.enum(["left", "right"]).optional().describe("Optional alignment override for this column"),
				}),
			)
...
function MetricGroupComponent({ props: { metrics } }: { props: { metrics: Array<MetricGroupItem> } }) {
	return (
		<div className="grid w-full grid-cols-2 gap-2">
			{metrics.map((metric, index) => (
				<Card key={metric.label} className="gap-1 py-3">
					<CardContent className="flex flex-col gap-1 px-3">
						<span className="text-[0.6875rem] font-medium text-foreground/50">{metric.label}</span>
...
export const MetricGroupDef = defineComponent({
	name: "MetricGroup",
	description: "A compact group of KPI cards with optional deltas and trend sparklines.",
	props: z.object({
		metrics: z
			.array(
				z.object({
					...
					delta: z.string().optional().describe("Optional period-over-period change label"),
					deltaTone: z.enum(["up", "down", "neutral"]).optional().describe("Visual tone for the delta"),
					sparkline: z.array(z.number()).optional().describe("Optional values for a compact trend sparkline"),
				}),
			)
