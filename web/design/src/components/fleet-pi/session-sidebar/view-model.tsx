...
export function useFleetSessionSidebarViewModel({
	...
	const sidebarItems = useMemo<SidebarResource[]>(() => {
		const grouped: SidebarResource[] = sortedProjects.map((project) => {
			const sessionsForProject = projectSessions.filter((session) => session.projectId === project.projectId);
		...
		const unassigned = sortSessions(projectSessions.filter((session) => !session.projectId));
