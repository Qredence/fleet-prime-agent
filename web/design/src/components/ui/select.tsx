...
type SelectProps = Omit<SelectPrimitive.Root.Props<string>, "children" | "items" | "onValueChange" | "value"> & {
	"aria-label"?: string;
	className?: string;
...
function Select({ className, onValueChange, options, placeholder, triggerId, value, ...props }: SelectProps) {
								...
								{options.map((option) => {
									...
									return (
										...
										>
											...
											{OptionIcon ? <OptionIcon className="size-4 shrink-0 text-muted-foreground" /> : null}
