...
export function sanitizeChartColor(value: string): string | undefined {
	...
	if (/^(?:rgb|rgba|hsl|hsla)\([^;{}<>]+\)$/i.test(color) && !/[;{}<>]/.test(color)) {
		return color;
	}
	return undefined;
}
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
	...
	const colorConfig = Object.entries(config).filter(([, itemCfg]) => itemCfg.theme ?? itemCfg.color);
...
${colorConfig
	.map(([key, itemConfig]) => {
		...
		const safeColor = typeof color === "string" ? sanitizeChartColor(color) : undefined;
...
type ChartTooltipContentProps = RechartsPrimitive.DefaultTooltipContentProps<TooltipValueType, TooltipNameType>;
				...
				return [
					...
					>
						{itemConfig?.icon && !hideIcon ? (
							...
							<div
								className="size-2 shrink-0 rounded-[2px] bg-(--chart-swatch)"
								style={
									{
										"--chart-swatch": item.color,
									} as React.CSSProperties
								}
							/>
						)}
