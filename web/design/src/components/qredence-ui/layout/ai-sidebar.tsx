...
export type SidebarResourceKind = "folder" | "project" | "file" | "bookmark" | "action";
...
function removeResource(items: SidebarResource[], id: string): { items: SidebarResource[]; removed?: SidebarResource } {
...
function moveResource(items: SidebarResource[], move: SidebarResourceMove): SidebarResource[] | null {
	...
	if (move.position === "inside" && (!target || target.disabled || !canContain(target))) return null;
 ...
 */
function ResourceActionMenu({ item, menuOpen, onMenuOpenChange, onRenameStart, renderMenu }: ResourceActionMenuProps) {
	...
	if (renaming) {
		return (
			<>
				...
				<ResourceRenameInput label={label} onCommit={onCommit} onCancel={onCancel} />
			</>
		);
	}
...
function ResourceRow(props: ResourceRowProps) {
	...
	const icon = renderedIcon === undefined ? defaultIcon(props.row.item, props.expanded) : renderedIcon;
			...
			>
				...
				{draggingId ? (
					<div
						...
						className="absolute inset-x-1 bottom-0 flex h-8 items-center justify-center rounded-lg border border-dashed border-border text-micro text-muted-foreground data-[active=true]:border-primary/50 data-[active=true]:bg-primary/10 data-[active=true]:text-foreground"
					>
						Move to top level
					</div>
				) : null}
...
export function AISidebar({
	...
	const flat = useMemo(() => flattenResources(renderedItems, expandedIds), [expandedIds, renderedItems]);
	...
	return (
		<ResourceList
			...
			onDragOver={(event, targetRow) => {
				...
				const position =
					!targetRow.item.disabled && canContain(targetRow.item) && ratio >= 0.25 && ratio <= 0.75
						? "inside"
						: ratio < 0.5
							? "before"
							: "after";
