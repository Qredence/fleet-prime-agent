...
export function Citation({ citationId, index, idPrefix, className }: CitationProps) {
	return (
		<a
			...
			className={cn(
				"mx-0.5 inline-flex min-w-4 -translate-y-0.5 items-center justify-center rounded-md bg-muted/60 px-1 py-0.5 text-micro font-semibold leading-none text-muted-foreground no-underline outline-none transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring",
				className,
			)}
		>
			{index}
	...
	const content = (
		<>
			...
			<span className="flex shrink-0 items-center gap-1.5">
				<span className="grid size-5 place-items-center rounded-md bg-foreground/[0.05] text-micro font-semibold tabular-nums text-muted-foreground">
					{index}
				</span>
				{safeUrl ? (
	...
	return (
		<div className={cn("w-full text-sm", className)}>
			...
			>
				...
				<span className="rounded-full bg-muted px-1.5 py-0.5 text-micro font-semibold tabular-nums">
					{citations.length}
				</span>
				<m.span
					aria-hidden="true"
