...
import { AgentDisclosure } from "@prime-agent/web-design/components/registry/beui/agents/agent-disclosure";
import { EASE_OUT, SPRING_LAYOUT, SPRING_SWAP } from "@prime-agent/web-design/lib/ease";
import { getFaviconUrl } from "@prime-agent/web-design/lib/favicon";
...
export function Citation({ citationId, index, idPrefix, className }: CitationProps) {
	return (
		<a
			...
			className={cn(
				"mx-0.5 inline-flex min-w-4 -translate-y-0.5 items-center justify-center rounded-md bg-muted/60 px-1 py-0.5 text-[0.625rem] font-semibold leading-none text-muted-foreground no-underline outline-none transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring",
				className,
			)}
		>
			{index}
...
export function CitationFavicon({ url, className }: { url?: string; className?: string }) {
	const favicon = url && isSafeExternalUrl(url) ? getFaviconUrl(url) : null;
...
function CitationRow({ citation, index, idPrefix }: { citation: CitationItem; index: number; idPrefix: string }) {
	const safeUrl = citation.url && isSafeExternalUrl(citation.url) ? citation.url : undefined;
	const content = (
		<>
			...
			<span className="flex min-w-0 flex-1 flex-wrap items-baseline gap-x-2 gap-y-0.5">
				...
				{citation.domain ? (
					<span className="min-w-0 truncate text-xs text-muted-foreground/60">{citation.domain}</span>
				) : null}
			</span>
			<span className="flex shrink-0 items-center gap-1.5">
				<span className="grid size-5 place-items-center rounded-md bg-foreground/[0.05] text-[0.625rem] font-semibold tabular-nums text-muted-foreground">
					{index}
				</span>
				{safeUrl ? (
	...
	return safeUrl ? (
		<a id={id} href={safeUrl} target="_blank" rel="noreferrer noopener" className={className}>
			{content}
		</a>
	) : (
	...
	return (
		<div className={cn("w-full text-sm", className)}>
			...
			>
				...
				<span className="rounded-full bg-muted px-1.5 py-0.5 text-[0.625rem] font-semibold tabular-nums">
					{citations.length}
				</span>
				<m.span
					aria-hidden="true"
