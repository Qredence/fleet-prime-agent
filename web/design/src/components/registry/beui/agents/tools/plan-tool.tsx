			...
			<div className="flex h-7 items-center justify-between pr-2.5 pl-3">
				<div className="flex min-w-0 items-center gap-1">
					{isPending ? (
						...
						<FileText className="h-3.5 w-3.5 shrink-0 text-an-tool-color-muted" />
					)}
					<span className="truncate text-xs text-an-tool-color-muted">{fileName}</span>
				</div>
				<button
					type="button"
			...
			<div className="border-t border-border bg-background pt-2">
				<div className="flex flex-col gap-1.5">
					<div className="flex items-center justify-between gap-2 px-3">
						...
						{actionLabel ? (
							<div className="rounded-full border border-border px-2 py-0.5 text-caption text-an-tool-color-muted">
								{actionLabel}
							</div>
						) : null}
					</div>
				...
				{(isExpanded || !hasSummary) && (
					<div className="mt-2 flex items-center justify-between border-t border-border bg-an-tool-background pt-1.5 pr-2 pb-2 pl-3.5">
						...
						{canAct ? (
							<div className="text-caption text-an-tool-color-muted">Choose what to do with this plan.</div>
						) : isAlreadyApproved ? (
							<div className="text-caption text-an-tool-color-muted">Approved</div>
						) : null}
					</div>
				)}
						...
						{isRefining ? (
							<div className="flex flex-col gap-2">
								<textarea
									...
									className="w-full resize-y rounded-[8px] border border-border bg-background px-3 py-2 text-base text-an-tool-color outline-none focus:border-an-primary-color md:text-sm"
								/>
