import { type ClipboardEvent, type KeyboardEvent, type MouseEvent, type ReactNode, type RefObject, useId } from "react";
...
export function InputBarSurface({ attachments, editor, layout, toolbar }: InputBarSurfaceProps) {
	const {
		attachedFiles,
		attachedImages,
		enableImagePreview,
	...
	const { infoBar, infoBarPosition, isDragOver, onContainerClick, questionBar, showInfoBar } = layout;
	const { attachRight, hasInput, isStreaming, leftActions, onAttach, onStop, onSubmit, rightActions, showAttach } =
		toolbar;
			...
			>
				...
				<div className="flex items-center justify-between gap-3 px-2 pt-1 pb-2">
					<div className="flex min-w-0 items-center gap-1">
						{!attachRight && showAttach && onAttach ? <AttachmentButton onClick={onAttach} /> : null}
						{leftActions}
					</div>
