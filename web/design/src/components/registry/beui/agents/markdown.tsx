...
const LazyHighlightedMarkdown = lazy(() =>
	import("./markdown-code").then(({ HighlightedMarkdown }) => ({
...
export const markdownComponents: Components = {
	...
	th: ({ children, ...props }) => (
		<th className="bg-an-background-secondary px-3 py-2 text-left font-medium" {...props}>
			{children}
		</th>
	),
	td: ({ children, ...props }) => (
		<td className="border-t border-an-border-color px-3 py-2 text-an-foreground/80" {...props}>
			{children}
		</td>
	),
};
...
export function Markdown(props: MarkdownProps) {
	if (!props.content.includes("```")) return <PlainMarkdown {...props} />;
