...
import { ActionSwapRollText } from "@prime-agent/web-design/components/registry/beui/motion/action-swap-roll";
import { Button } from "@prime-agent/web-design/components/registry/beui/motion/button/index";
import { Checkbox } from "@prime-agent/web-design/components/registry/beui/motion/checkbox";
import { RadioGroup, RadioGroupItem } from "@prime-agent/web-design/components/registry/beui/motion/radio";
import { ValidatedInput } from "@prime-agent/web-design/components/registry/beui/motion/validated-input";
	...
	return (
		...
		>
			<div className="flex items-start gap-3">
				<span
					aria-hidden="true"
					className={cn("grid size-5 shrink-0 place-items-center text-muted-foreground", getStatusClass(status))}
				>
					{busy ? (
				...
				<div className="min-w-0 flex-1">
						...
						<AnimatePresence initial={false} mode="wait">
							{questionMode && question ? (
								...
								<m.div key="description" initial={false}>
									{description ? <p className="mt-1 leading-5 text-muted-foreground">{description}</p> : null}
					...
					{!interactive ? <p className="mt-1 text-sm text-muted-foreground">{result ?? statusLabel}</p> : null}
				</div>
			</div>
		</div>
	);
