...
function InputSuggestionsOverlay({
	...
	const filteredSlashCommands = useMemo(() => {
		...
		return slashCommandConfig.items.filter((item) =>
			`${item.id} ${item.label} ${item.value ?? ""}`.toLowerCase().includes(slashQuery),
		);
	}, [slashCommandConfig.items, slashQuery]);
	const interactionsDisabled = disabled || isStreaming;
	const showSlashCommands = filteredSlashCommands.length > 0 && !interactionsDisabled && !slashDismissed;
	...
	const [prevSlashCount, setPrevSlashCount] = useState(filteredSlashCommands.length);
