...
import type { InputHTMLAttributes, KeyboardEvent as ReactKeyboardEvent, ReactNode, Ref } from "react";
...
export function ComboboxTrigger({ children, className, showIndicator = true }: ComboboxTriggerProps) {
...
export function ComboboxValue({ placeholder = "Select an option", children, className }: ComboboxValueProps) {
	const context = useComboboxContext("ComboboxValue");
	const label = context.labelFor(context.value);
	const content = typeof children === "function" ? children(context.value, label) : (children ?? label ?? placeholder);
...
export interface ComboboxInputProps extends Omit<InputHTMLAttributes<HTMLInputElement>, "defaultValue" | "value"> {
