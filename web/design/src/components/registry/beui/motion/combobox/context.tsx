...
export function Combobox({
	...
	const updateOpen = useCallback(
		(next: boolean, restoreFocus = false) => {
			...
			onOpenChange?.(next);
			if (!next) updateQuery("");
			if (restoreFocus) requestAnimationFrame(() => inputRef.current?.focus({ preventScroll: true }));
		},
	...
	const visibleItems = useMemo(
		() => Array.from(items.values()).filter((item) => filter(item.value, query, [item.label, ...item.keywords])),
		[filter, items, query],
	);
	...
	const moveActive = useCallback(
		(direction: 1 | -1 | "first" | "last") => {
			...
			const currentIndex = enabledVisibleItems.findIndex((item) => item.value === activeValue);
			const nextIndex =
				currentIndex < 0
					? direction === 1
						? 0
	...
	useEffect(() => {
		...
		const activeVisible = activeValue && visibleValues.has(activeValue) ? activeValue : null;
		setActiveValue(activeVisible ?? selectedVisible ?? enabledVisibleItems[0]?.value ?? null);
	...
	useEffect(() => {
		if (!open) return;
		const isInside = (target: Node) => rootRef.current?.contains(target) || contentRef.current?.contains(target);
