...
export function ComposerCommandItem({
	...
	return (
		<ComposerMenuItem active={active} {...props}>
			...
			{active && (
				<kbd className="bg-foreground/[0.06] text-foreground/45 rounded px-1 font-mono text-[0.625rem]">↵</kbd>
			)}
		</ComposerMenuItem>
	);
}
...
export function ComposerVoice({
	...
	return (
		...
		>
			{recording && (
				<span aria-hidden className="size-1.5 animate-pulse rounded-full bg-blue-500 dark:bg-blue-400" />
			)}
			...
			{recording ? (
				...
				<ShimmerLabel className="text-foreground/55 relative text-[0.8125rem]">Transcribing</ShimmerLabel>
			)}
		</div>
	);
}
...
export function ComposerToolbar({ className, ...props }: ComponentProps<"div">) {
	return (
		<div data-slot="composer-toolbar" className={cn("flex items-center justify-between", className)} {...props} />
	);
}
		...
		<div data-slot="composer-context" className={cn("group/ctx relative", className)} {...props}>
			...
			>
				...
				<div className="flex flex-col gap-2">
					{segments.map((segment) => (
						<div key={segment.label} className="text-foreground/55 flex items-center gap-2.5 text-[0.8125rem]">
