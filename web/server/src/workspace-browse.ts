 ...
 */
export async function browseWorkspaceDirectories(rawPath: string): Promise<WorkspaceBrowseResult> {
	...
	try {
		...
		return {
			kind: "error",
			status: 403,
			message: error instanceof Error ? error.message : `Could not read directory: ${path}`,
		};
	}
