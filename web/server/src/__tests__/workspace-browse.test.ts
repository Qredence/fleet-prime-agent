...
describe("workspace-browse", () => {
	...
	it("lists child directories and excludes files and dotdirs", async () => {
		...
		expect(result.entries.map((entry) => entry.name).sort()).toEqual(["apps", "packages"]);
	...
	it("includes symlink-to-directory entries", async () => {
		...
		expect(result.kind).toBe("ok");
		if (result.kind !== "ok") return;
		expect(result.entries.map((entry) => entry.name).sort()).toEqual(["apps", "packages", "packages-link"]);
	});
